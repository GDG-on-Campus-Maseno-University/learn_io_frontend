...
const Carossel = ({ images, speed = 100, transitionDuration = 5000 }) => {
  ...
  return (
    <div className="carousel-wrapper">
      <button className="left-arrow" onClick={() => handleManualScroll('left')}>&#8249;</button>
      <div ref={sliderRef} className="slider-container">
        {images.concat(images).map((image, index) => (
          <img
            ...
            style={{ transition: `transform ${transitionDuration}ms ease-in-out` }}  // Add smooth transition
          />
        ))}
      </div>
      <button className="right-arrow" onClick={() => handleManualScroll('right')}>&#8250;</button>
    </div>
  );
};
